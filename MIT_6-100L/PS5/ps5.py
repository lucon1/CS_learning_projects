...
def img_to_pix(filename):
    ...
    return list(input_image.getdata())
...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        added_pixel = matrix_multiply(color_matrix, pixel)
        for i in range(len(added_pixel)):
            added_pixel[i] = int(added_pixel[i])
        new_pixels.append(tuple(added_pixel))
    return new_pixels
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if type(pixel) == tuple:
        end_bits = []
        for num in pixel:
            end_bits.append(num % 2**num_end_bits)
        return tuple(end_bits)
...
def reveal_bw_image(filename):
    ...
    originl_pixels = img_to_pix(filename)
    hidden = []
    num_end_bits = 1
    for pixel in originl_pixels:
        hidden.append(extract_end_bits(num_end_bits, pixel) * 255)
    return pix_to_img(hidden, original_img.size , original_img.mode)
...
def reveal_color_image(filename):
    ...
    revealed = []
    num_end_bits = 3
    for i in range(len(originl_pixels)):
        hidden.append(list(extract_end_bits(num_end_bits, originl_pixels[i])))
        for j in range(len(hidden[i])):
            hidden[i][j] = int(hidden[i][j] * 255 / ((2**num_end_bits)-1))
        revealed.append(tuple(hidden[i]))
    return pix_to_img(revealed, original_img.size , original_img.mode)
